...
void drawDensity(int n , double x, double y, TypeInfoDrawTreeGeneric *info, void *data) {
	if(data && ((TypeDataDrawDensity*)data)->dens && ((TypeDataDrawDensity*)data)->dens[n].size>0) {
		...
		ind = 0;
		xtab[ind] = (((TypeDataDrawDensity*)data)->dens[n].item[0].val-info->param.tmin)*xscale+xoffset;
		ytab[ind] = (1-2.*PER)*info->param.leafSep+yoffset;
		ind++;
		for(i=0; i<((TypeDataDrawDensity*)data)->dens[n].size; i++) {
		...
		//ytab[ind] = (1-2.*PER)*info->param.leafSep+yoffset;
		xtab[ind] = (2*((TypeDataDrawDensity*)data)->dens[n].item[((TypeDataDrawDensity*)data)->dens[n].size-1].val-((TypeDataDrawDensity*)data)->dens[n].item[((TypeDataDrawDensity*)data)->dens[n].size-2].val-info->param.tmin)*xscale+xoffset;
		ytab[ind] = (1-2.*PER)*info->param.leafSep+yoffset;

		((TypeDataDrawDensity*)data)->fillPolygon(((TypeDataDrawDensity*)data)->color, ((TypeDataDrawDensity*)data)->alpha, xtab, ytab, ((TypeDataDrawDensity*)data)->dens[n].size+2, &(info->param));
		free((void*)xtab);
		free((void*)ytab);
	}
}
