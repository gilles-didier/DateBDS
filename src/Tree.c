...
void fillDepth(int node, int curDepth, TypeTree *tree, int *depth) {
	...
	for(c=tree->node[node].child; c!=NOSUCH; c=tree->node[c].sibling)
		fillDepth(c, curDepth+1, tree, depth);
}
...
int readNode(FILE *f, TypeTree *tree) {
    ...
    if(c == ')') {
        ...
        tree->comment[current] = NULL;
        tree->comment[current] = readComment(f);
		tree->time[current] = readTime(f);
		tmp = readComment(f);
		if(tmp != NULL) {
			if(tree->comment[current] == NULL)
				tree->comment[current] = tmp;
			else
				free((void*)tmp);
		}
    } else {
...
void reorderTreeRec(char **tmp, char **name, int n, TypeTree *tree) {
    ...
    if(nchild>0) {
        ...
        tree->node[tab[nchild-1].index].sibling = NOSUCH;
        free((void*)tab);
    }
    if(nchild>0)
...
TypeTree *pruneLeavesFromTable(TypeTree *tree, int *keep) {
	TypeTree *resT;
	int n, *new, *parent;
	...
	for(n=0; n<tree->size; n++) {
		if(tree->node[n].child == NOSUCH && keep[n]) {
			int m, prec;
			new[n] = resT->size++;
			prec = new[n];
